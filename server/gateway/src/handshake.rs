...
impl ControlMessage {
    ...
    pub fn decode(input: &[u8]) -> io::Result<Self> {
        ...
        match kind {
            CLIENT_HELLO => Ok(Self::ClientHello { session_id: u64_at(input, 1)? }),
            SESSION_ACCEPT => Ok(Self::SessionAccept { session_id: u64_at(input, 1)? }),
            PATH_REGISTER => Ok(Self::PathRegister { session_id: u64_at(input, 1)?, path_id: u32_at(input, 9)? }),
            PATH_ACK => Ok(Self::PathAck { session_id: u64_at(input, 1)?, path_id: u32_at(input, 9)? }),
            HEARTBEAT => Ok(Self::Heartbeat { session_id: u64_at(input, 1)?, path_id: u32_at(input, 9)?, nonce: u64_at(input, 13)? }),
            HEARTBEAT_ACK => Ok(Self::HeartbeatAck { session_id: u64_at(input, 1)?, path_id: u32_at(input, 9)?, nonce: u64_at(input, 13)? }),
            PATH_CLOSE => Ok(Self::PathClose { session_id: u64_at(input, 1)?, path_id: u32_at(input, 9)? }),
            SESSION_CLOSE => Ok(Self::SessionClose { session_id: u64_at(input, 1)? }),
            _ => Err(io::Error::new(io::ErrorKind::InvalidData, "unknown control message")),
        }
    }
}
